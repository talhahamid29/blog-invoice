...
export default function MobileNavbar({ navigationItems, open }) {
  ...
  return (
    <div
      className={` md:hidden block bg-gray-100 fixed w-full top-20 overflow-y-auto bottom-0 py-6 duration-500 ${open ? "left-0" : "left-[-100%]"}`}>
      {navigationItems.map((item, index) => (
        <div key={index}>
          <h1
            className={`${
              item.current || isActive(item.href) ? "text-indigo-600" : "text-gray-700"
          ...
          <div className={`${label === item.label && show ? "md:hidden" : "hidden"} flex flex-col bg-gray-200  transition duration-500 `}>
            {item.sublinks.map((sublink, index) => (
              <Disclosure.Button
                ...
                className={`${
                  isActive(sublink.href) ? "text-indigo-600 font-bold" : "text-gray-700"
                } py-3 pl-14 hover:font-bold  hover:bg-indigo-500 hover:text-white`}>
                {sublink.name}
              </Disclosure.Button>
            ))}
          </div>
